...
export async function updateUserAdmin(
    ...
    const {error: authError} = await admin.auth.admin.updateUserById(user_id, {email, app_metadata: {user_role: role}});

    if (authError) {
        return {
            errors: {
                _form: [authError.message],
                ...(authError.message.toLowerCase().includes("email") ? {email: [authError.message] } : {}),
            }
        };
    }
...
export async function createUserAdmin(
    ...
    if (authError || !authData.user) { 
        return { errors: { _form: [authError?.message ?? "Could not create user"] } };
    } 
